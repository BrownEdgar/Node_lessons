...
class ProductServices {
  ...
  async getProductsByType(res, type) {
    const result = await this.models.products.aggregate([{ $match: { type: type } }])
    return result.length ? { message: ErrorMessages.TYPE_ERROR } : result
  }

  async getProductShoptById(res, id) {
    const specialProduct = await this.models.products.aggregate([{ $match: { _id: mongoose.Types.ObjectId(id) } }, { $project: { name: 1, price: 1, _id: 0 } }])
  ...
  async updateAllProduct(res, body, id) {
    const newProduct = await this.models.products.findOneAndUpdate({ _id: mongoose.Types.ObjectId(id) }, { $set: { ...body } })
