...
class WineController {
  ...
  async getAllWines(req, res) {
    try {
      ...
      const addWines = await req.app.services.wines.getAllWines(res);
  ...
  async addSingleWine(req, res) {
    try {
      const singleWine = await req.app.services.wines.addSingleWine(res, req.body);
  ...
  async getWineByName(req, res) {
    ...
    try {
      const wname = await req.app.services.wines.getWineByName(res, winename);
  ...
  async getWinesByMultipleFields(req, res) {
    ...
    try {
      const response = await req.app.services.wines.getWinesByMultipleFields(res, winename, price);
  ...
  async getWineByCompanyName(req, res) {
    ...
    try {
      const response = await req.app.services.wines.getWineByCompanyName(res, companyname);
  ...
  async getMostExpensiveWine(req, res) {
    try {
      const response = await req.app.services.wines.getMostExpensiveWine(res);
      res.status(200).json({ message: response });
    } catch (err) {
      res.status(500).send(err.message);
    }
  ...
  async priceIncrease(req, res) {
    ...
    try {
      const response = await req.app.services.wines.priceIncrease(res, sum);
      res.status(200).json({ message: response });
    } catch (err) {
      res.status(500).send(err.message);
    }
  ...
  async randomWine(req, res) {
    try {
      const response = await req.app.services.wines.randomWine(res);
      res.status(200).json({ message: response });
    } catch (err) {
      res.status(500).send(err.message);
    }
